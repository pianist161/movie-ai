...
import MovieChat from '@/common/MovieChat/MovieChat';
...
const Detail = () => {
  ...
  useEffect(() => {
    document.title =
      (movie?.title || movie?.name) && !isLoading ? movie.title || movie.name : 'tMovies';
      ...
      <section className='w-full' style={backgroundStyle}>
        ...
        >
          ...
          >
            ...
            >
              💬 Chat with AI about this movie
            </button>

            {chatOpen && (
              <MovieChat movieTitle={movie.title} onClose={() => setChatOpen(false)} />
            )}
